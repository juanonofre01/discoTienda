...
public class Discotienda
{
    ...
    {
        for( int l = 0; l < discos.size( ); l++ )
        {
            Disco d1 = ( Disco )discos.get( l );
            for( int n = l + 1; n < discos.size( ); n++ )
            {
                Disco d2 = ( Disco )discos.get( n );
    ...
    // -----------------------------------------------------------------
    // Trabajo estudiantes Juan Angel Achicanoy Onofre - Nicolas David Benavides Rivera
    // -----------------------------------------------------------------
    ...
    public void generarInformeDiscos() throws FileNotFoundException
    {
    	...
    	// Escribir un reporte de discos con nombre, artista y g�nero
    	// ==========================================================
    	
    	// Recorrido total sobre discos
    	for(int i = 0; i < discos.size(); i++)
    	{
    		//Extraer informaci�n de cada disco
    		Disco miDisco = (Disco)discos.get(i);
    		
    		//Escribir con la pluma la informaci�n
    		pluma.println("Nombre: " + miDisco.darNombreDisco() +
    					  "Artista: " + miDisco.darArtista() + 
    					  "G�nero: " + miDisco.darGenero());
     ...
     * @return Retorna un informe con los discos de g�nero ROCK y POP que no superan los 1000 pesos.
     * @throws Exception 
     */
    ...
    public void generarInformeGenerosRockPop() throws Exception
    {
    	...
    	// Excepci�n utilizada cuando no se agrega ningun disco
    	if(estado == false){
    		throw new Exception("No hay discos de genero Rock o Pop que no superen los 1000 pesos");
    	}
    ...
    public String metodo1( )
    {
        try {
        	...
        	return "Reporte generado satisfactoriamentee";
        } catch (Exception e) {
